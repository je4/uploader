	...
	vfsStore := &VFSStore{
		...
		uploadCache: gcache.New(100).LRU().Expiration(cacheExpiration).EvictedFunc(func(key any, value any) {
			...
			if err := upload.Stop(); err != nil {
				logger.Error().Err(err).Msgf("cannot stop upload %v", key)
				return
			}
		}).Build(),
		logger: logger,
...
func (store VFSStore) NewUpload(ctx context.Context, info handler.FileInfo) (handler.Upload, error) {
	...
	if info.Storage != nil && info.Storage["defaultPath"] != "" {
		...
		if strings.HasPrefix(info.Storage["defaultPath"], "vfs:") {
			...
			binPath = store.uploadPath + "/" + filepath.ToSlash(filepath.Clean(info.Storage["defaultPath"]))
		}
	} else {
	...
	upload, err := NewUpload(binPath, &store, info, store.logger)
	if err != nil {
		return nil, errors.Wrapf(err, "cannot create upload %v", info.ID)
	}

	store.uploadCache.Set(upload.info.ID, upload)
...
func (store VFSStore) GetUpload(ctx context.Context, id string) (handler.Upload, error) {
	...
	if upload.file == nil {
		binPath := store.defaultBinPath(id)
		fi, err := fs.Stat(store.fsys, binPath)
		if err != nil {
			return nil, errors.Wrapf(err, "cannot stat file '%s'", binPath)
		}
		if fi.IsDir() {
			return nil, errors.Errorf("cannot get upload '%s': path is a directory", binPath)
		}
		fp, err := writefs.Append(store.fsys, binPath)
		if err != nil {
			return nil, errors.Wrapf(err, "cannot append file '%s'", binPath)
		}
		upload.file = fp
	}
	return upload, nil
